...
public enum CellState: String, CustomStringConvertible{
    ...
    public static var allValues: [CellState] = [.alive, .empty, .born, .died]
...
public extension GridProtocol {
    public func getNumDif(of type: CellState, vs grid: GridProtocol) -> Int{
        return lazyPositions(self.size).reduce(0){ self[$1.row, $1.col] == type && grid[$1.row,$1.col] != type ? $0+1: $0}
    }
    public func newGrid(with points:[(Int, Int)]) -> GridProtocol{
        let maxR = points.reduce(0){ $0 < $1.0 ? $1.0 : $0 }
        let maxC = points.reduce(0){ $0 < $1.1 ? $1.1 : $0 }
        
        var nGrid: GridProtocol = Grid(maxR > 50 ? 100 : maxR * 2, maxC > 50 ? 100 : maxC * 2)
        _ = points.map{nGrid[$0.0,$0.1] = .alive}
        return nGrid
    }
    public func getAlive() -> [(Int, Int)]{
        return lazyPositions(self.size).filter {return  self[$0.row, $0.col].isAlive}
    }
}
